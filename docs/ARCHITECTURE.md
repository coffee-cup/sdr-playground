...
- **`decode`**: decoder tails. Defines the `Decoder` trait and `Event` enum, and contains the RDS decoder (pilot-locked 57 kHz recovery, biphase symbol sync, block sync + error detection, group parsing → PI/PS/RadioText/PTY). Depends on `dsp`. Tested against synthetic signals and recorded fixtures.
