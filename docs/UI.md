...
`sdr-scan scan` runs a `ratatui` table that fills in as the scanner sweeps: one row per station with RDS, showing frequency, the program-service name, the program type, and what is playing. The now-playing column prefers RT+ (RadioText Plus), which carries a clean tagged "Artist - Title" carved out of the free-form RadioText, and falls back to the raw RadioText when a station does not send RT+. The station column shows the RDS2 Long PS name when broadcast (otherwise the 8-character PS), and the type column shows the station's own PTYN label when broadcast (otherwise the standard program-type name); clock-time is decoded too. A status line shows the window being tuned. The band is covered as a sequence of device-bandwidth windows because one RTL-SDR sees only ~1-2 MHz at once. Dwell is adaptive: a window is held while it keeps yielding new RDS and left once it falls quiet (`--quiet` seconds, default 2) or hits a cap (`--dwell`), so empty stretches of the band cost a couple of seconds rather than a fixed dwell and a full sweep is several times faster. Slow-arriving RadioText keeps filling in across the repeated sweeps. `sdr-scan scan --print` runs one headless, self-timing pass (the scan benchmark); `sdr-scan probe --freq <MHz>` is the headless single-station path used to validate decoding against the live device.
